...
class PlayerWinInteractions(object):
    ...
    def select_point(self, x, y):
        ...
        """
        if self._polybtn.checked or self._circlebtn.checked or self._rectbtn.checked:
        ...
        if index is not None:
            try:
                ...
                for point_index, point in enumerate(points):
                    ...
                    distance = get_intersection_point_distance(mouse, point, next_point)
    ...
    def on_player_click_in_video_window(self, event, x, y):
        ...
        mouse = int(x), int(y)
        self._selected_point = None
        if not self._rectbtn.checked and not self._polybtn.checked and not self._circlebtn.checked and not self._add_points_btn.checked:
            self.select_point(*mouse)

        if self._polybtn.checked or self._circlebtn.checked or self._add_points_btn.checked:
            ...
            if index is not None:
                ...
                if len(points) >= 5 and self._circlebtn.checked:
                    ...
                    points_list.set_value(0, index, str(points.tolist())[1:-1] + ',')
